...
| 4 | 溯源信息 | 商品建档与流通信息 | 商品ID、溯源码、批次号、建档日期、采收/出栏日期、包装日期、检测说明、基础溯源字段、分类扩展字段 |
...
| 9 | trace_code | 溯源码 | varchar(64) | 业务上要求唯一，当前库层允许为空 | 用于二维码访问独立溯源档案 |
| 10 | batch_no | 批次号 | varchar(64) | 可空 | 标识商品建档批次 |
| 11 | trace_extra | 溯源扩展字段 | text | JSON 文本 | 存储不同分类的特色溯源字段 |
| 12 | transfer_status | 打款状态 | tinyint | 0~3 | 控制对账打款流程 |
...
| 主表 | 从表 | 关联字段 | 关系说明 |
| ---- | ---- | ---- | ---- |
| `user_info` | `user_address` | `user_id` | 一个用户可维护多个收货地址 |
| `order_main` | `order_item` | `order_no` | 一个订单对应多条商品明细 |
| `order_main` | `payment_record` | `order_no` | 一笔订单对应一条支付记录 |
| `order_main` | `logistics_info` | `order_no` | 一笔订单对应一条物流记录 |
| `order_main` | `after_sale` | `order_no` | 一个订单可派生售后记录 |
| `after_sale` | `after_sale_communication` | `after_sale_no` | 一个售后单对应多条沟通消息 |
| `product_info` | `product_trace` | `product_id` | 一个商品对应一条溯源档案 |
| `merchant_info` | `shop_info` | `merchant_id` | 一个商家对应一个店铺信息 |
| `sys_role` | `sys_role_permission` | `role_id` | 角色与权限关联 |
| `sys_permission` | `sys_role_permission` | `perm_id` | 权限被多个角色引用 |
...
| 业务主键 | product_id |
| 主要索引 | `uk_product_id` |
| 作用 | 存储独立溯源码、建档批次、基础溯源字段、分类特色溯源字段和二维码信息 |

| 序号 | 中文名称 | 列名 | 数据类型 | 非空 | 说明 |
| ---- | ---- | ---- | ---- | ---- | ---- |
| 1 | 溯源ID | id | bigint | 是 | 主键 |
| 2 | 商品ID | product_id | bigint | 是 | 与商品一对一关联 |
| 3 | 溯源码 | trace_code | varchar(64) | 否 | 采用 `TRACE-` 前缀业务编号 |
| 4 | 批次号 | batch_no | varchar(64) | 否 | 商品建档批次 |
| 5 | 生产建档日期 | production_date | date | 否 | 建档日期 |
| 6 | 采收/出栏日期 | harvest_date | date | 否 | 农产品采收或畜禽出栏时间 |
| 7 | 包装入库日期 | packaging_date | date | 否 | 包装/入库时间 |
| 8 | 检测说明 | inspection_report | varchar(500) | 否 | 检测或检疫结果 |
| 9 | 基础溯源字段 | planting_cycle / origin_place_detail / fertilizer_type / storage_method / transport_method | 多字段 | 否 | 用于统一展示基础溯源信息 |
| 10 | 分类扩展字段 | trace_extra | text | 否 | 使用 JSON 保存四类商品的差异化溯源字段 |
| 11 | 二维码地址 | qr_code_url | text | 否 | 保存二维码 Data URL 或访问地址 |
...
| 主键 | id |
| 业务主键 | 无 |
| 主要索引 | `uk_order_no`、`idx_pay_status` |
| 作用 | 存储模拟支付与退款信息 |
...
7. `product_trace` 表的扩展字段通过应用启动初始化器自动补齐，能够兼容项目迭代过程中新增的溯源字段。
8. `payment_record` 表的 `order_no` 在当前脚本中已设置唯一约束，用于支撑支付成功流程的幂等控制；对于历史环境，如缺少该约束，应通过初始化逻辑或手工 DDL 进行补齐。
9. `user_message`、`order_communication`、`product_trace.trace_extra` 等结构体现了数据库设计对“业务迭代后兼容扩展”的考虑。

### 4.3.4 关键约束与索引策略说明
为兼顾毕业设计项目的可实现性与业务正确性，数据库设计采用“少量强约束 + 服务层事务控制 + 高频索引优化”的组合策略。

#### 1. 唯一约束策略
1. 用户、商家、管理员登录标识采用唯一约束，避免账号重复。
2. 订单号、售后单号、提现单号等业务编号采用唯一约束，保证跨表关联稳定。
3. `product_trace` 通过 `uk_product_id` 保证商品与溯源档案一对一关系。
4. `payment_record` 通过 `order_no` 唯一约束保证“一笔订单一条有效支付记录”的设计目标。
5. `product_trace.trace_code` 当前主要由应用层按规则生成并在业务上保持唯一，数据库层现阶段重点保证“一个商品仅对应一条溯源档案”；如后续系统投入真实生产，可继续为 `trace_code` 增加独立唯一索引。

#### 2. 查询索引策略
1. 对用户 ID、商家 ID、订单状态、审核状态、创建时间等高频筛选条件建立普通索引。
2. 对 `business_type + business_id` 等组合查询场景建立联合索引，提升后台日志和审核记录查询性能。
3. 对 `merchant_id + status`、`status + retry_count` 等资金监管场景建立联合索引，支撑提现审核和失败重试。

#### 3. 一致性保障策略
1. 数据库层主要负责主键、唯一键和索引约束。
2. 复杂状态流转一致性由 Spring Boot 业务层事务统一维护。
3. 高并发下的库存扣减采用带条件的原子更新语句，避免“先查后改”带来的超卖问题。
...
| 表名 | 关键约束/索引 | 设计目的 |
| ---- | ---- | ---- |
| `sys_user` | `uk_sys_user_username`、`idx_role_id`、`idx_phone` | 保证管理员账号唯一，提升角色与手机号查询 |
| `user_info` | `uk_phone`、`idx_login_token` | 保证用户手机号唯一，提升登录态查询 |
| `merchant_info` | `uk_phone`、`idx_audit_status`、`idx_status` | 保证商家账号唯一，提升审核和状态检索 |
| `order_main` | `uk_order_no`、`idx_user_id`、`idx_merchant_id`、`idx_order_status` | 保证订单号唯一，支撑用户端/商家端/后台按状态查询 |
| `payment_record` | `uk_order_no`、`idx_pay_status` | 保证一单一支付记录，支撑支付与退款查询 |
| `after_sale` | `uk_after_sale_no`、`idx_order_no`、`idx_after_sale_status` | 保证售后业务主键唯一，支撑按订单与状态追踪 |
| `product_trace` | `uk_product_id` | 保证商品与溯源信息一对一，`trace_code` 当前主要由应用层生成规则保障业务唯一性 |
| `merchant_withdraw` | `uk_withdraw_no`、`idx_merchant_status`、`idx_status_retry` | 保证提现单唯一，支撑审核与重试流程 |
...
3. 下单扣减库存采用原子更新语句完成，核心条件包含 `stock >= quantity`，以降低并发场景下超卖风险。
4. 用户取消订单、超时取消订单和支付失败回滚时，系统通过库存恢复语句同步修正 `stock` 与 `sales_volume`。
5. 对账和提现流程通过状态字段、重试次数和失败原因共同描述执行过程，便于失败重试和人工兜底。
6. 支付成功流程通过支付记录唯一约束和订单状态校验共同实现幂等控制，避免重复支付请求导致重复写入。
7. 管理员退款处理通过当前退款状态校验进行收口，避免对已处理记录重复执行状态流转。
8. 备份采用全表数据导出策略，适合当前毕业设计和轻量运维场景。
9. 恢复操作基于备份 SQL 文件执行，恢复前应确认目标环境与表结构版本一致。
