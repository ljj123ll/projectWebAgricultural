...
7. 商品溯源二维码：基于商品溯源信息生成二维码链接，并跳转到独立的溯源档案页。
8. 分类溯源建模：通过基础溯源字段 + `trace_extra` 扩展字段实现“一物一码、多品类差异化溯源”。
9. 原子库存扣减：下单阶段通过带 `stock >= quantity` 条件的更新语句降低并发超卖风险。
...
## 2.5 系统分层与代码组织设计
为保证系统具有良好的可维护性和扩展性，本项目在实现层面采用了较为清晰的分层设计。

### 2.5.1 后端分层设计
后端采用典型的控制层、业务层、数据访问层和公共支撑层结构：

```mermaid
flowchart TD
    A[Controller 控制层] --> B[Service 业务层]
    B --> C[Mapper 数据访问层]
    B --> D[Common Service 公共能力]
    C --> E[(MySQL)]
    D --> F[(Redis)]
    D --> G[WebSocket]
    D --> H[定时任务]
...
1. 控制层：负责请求接收、参数绑定、登录校验、权限校验和统一结果返回。
2. 业务层：负责业务规则、状态流转、事务控制、缓存清理和事件发布。
3. 数据访问层：负责对数据库表进行增删改查。
4. 公共支撑层：负责缓存、实时推送、文件上传、短信验证码、地区与物流等通用能力。
5. 定时任务层：负责超时取消订单、自动打款、自动备份和提现打款重试。

### 2.5.2 前端分层设计
前端采用“路由 + 布局 + 页面 + API + 状态管理 + 工具函数”的组织方式：

1. `router`：统一定义用户端、商家端和管理员端的页面路由。
2. `layout`：按角色提供统一布局和导航结构。
3. `views`：承载具体业务页面。
4. `apis`：封装前端到后端的 HTTP 请求。
5. `stores`：存储登录态、购物车、订单等共享状态。
6. `utils`：提供图片处理、请求封装、实时消息解析等工具能力。

### 2.5.3 代码目录映射
| 分层 | 前端目录 | 后端目录 |
| ---- | ---- | ---- |
| 路由与布局 | `src/router`、`src/layout` | 无 |
| 业务页面 | `src/views/user`、`src/views/merchant`、`src/views/admin` | `controller/user`、`controller/merchant`、`controller/admin` |
| 接口封装 | `src/apis` | `controller/*` |
| 业务逻辑 | `src/stores`、页面组合逻辑 | `service/user`、`service/merchant`、`service/admin`、`service/common` |
| 数据访问 | 无 | `mapper`、`entity`、`vo`、`dto` |
| 配置与支撑 | `src/utils`、`src/constants` | `config`、`job`、`websocket`、`util` |
...
## 3.3 商品与溯源模块
### 3.3.1 模块描述
商品与溯源模块负责商品分类、商品发布、商品展示、商家推荐、独立溯源档案展示和一物一码溯源信息维护。
...
| product_trace | 商品溯源表 | 存储独立溯源码、基础溯源字段、分类扩展字段和二维码信息 |
| shop_info | 店铺信息表 | 商家店铺展示 |

### 3.3.4 输入与输出
输入：商品名称、分类、价格、库存、图片、溯源信息。  
输出：商品列表、商品详情、独立溯源档案、溯源二维码、商家推荐商品。

### 3.3.5 算法与规则
1. 商品详情页的商家推荐以当前商家销量较高的商品为基础。
2. 滞销专区展示结果由管理员手动配置和算法推荐共同决定。
3. 热门商品排行由 Redis ZSet 维护销量分值。
4. 商品保存时自动生成业务上唯一的 `TRACE-` 前缀溯源码，并与商品溯源记录绑定。
5. 溯源数据采用“基础字段 + 分类扩展字段”方式组织，其中扩展字段由 `TraceabilityCatalog` 按分类白名单清洗。
6. 商家端生成二维码时，二维码内容指向 `/trace/{traceCode}` 独立溯源档案页。
7. 商家主页的商家评分由该商家全部已审核评论汇总计算得到，评论区展示的是商家全部商品评论的聚合结果。
...
sequenceDiagram
    ...
    Service->>Service: 生成 traceCode 并规范化分类溯源字段
...
6. 为避免并发抢购下超卖，库存扣减采用带库存充足条件的原子更新语句完成，而不是“先查询再减库存”的两步式处理。
7. 单笔订单中的商品必须来自同一商家，便于后续商家履约、售后流转和资金结算。
8. 支付成功处理采用“状态校验 + 幂等更新”方式收口，同一订单在前端重复点击、请求重试时不会重复写入支付成功。
9. 前端支付、取消订单、商家发货、管理员退款等动作均增加提交中锁定，降低演示时因连续点击引起的异常。
...
5. 订单沟通消息支持文本、图片和视频三种消息类型，并单独存储于 `order_communication` 表中。
6. 同一订单在存在处理中售后单时，系统不再允许重复创建新的处理中售后，避免状态冲突。
...
5. 退款处理属于后台高风险操作，管理员仅能对“退款中”记录执行确认退款或退款失败流转。
...
| IF-PROD-02 | 溯源档案查询接口 | 商品详情页、二维码访问页 | 根据 `traceCode` 返回独立溯源档案 |
| IF-PROD-03 | 商家二维码生成接口 | 商家商品发布页 | 生成溯源二维码并保存到溯源记录 |
...
## 5.5 关键页面与接口映射
为便于将概要设计与实际实现进行对照，核心页面与后端接口、服务入口的映射关系如下：

| 业务场景 | 页面路由 | 关键页面函数 | 后端接口 | 后端服务入口 |
| ---- | ---- | ---- | ---- | ---- |
| 用户下单 | `/order-confirm` | `handleSubmit` | `POST /user/orders` | `UserOrderService.create` |
| 用户支付 | `/order-pay/:orderId` | `loadOrder`、`handlePay` | `POST /user/orders/{id}/pay` | `UserOrderService.pay` |
| 商家主页与评分 | `/shop/:merchantId` | `loadShop`、`loadComments` | `GET /user/merchants/{id}`、`GET /user/merchants/{id}/comments` | `UserMerchantService.getShop`、`UserMerchantService.listComments` |
| 独立溯源档案 | `/trace/:traceCode` | `loadTraceArchive` | `GET /user/traces/{traceCode}` | `UserProductService.getTraceArchive` |
| 商家发货 | `/merchant/order-detail/:orderNo` | `loadOrderDetail`、`confirmShip` | `PUT /merchant/orders/{id}/ship` | `MerchantOrderService.ship` |
| 管理员退款 | `/admin/payments` | `loadPayments`、`handleRefund` | `PUT /admin/payments/{id}/refund` | `AdminPaymentService.updateRefundStatus` |
| 用户申请售后 | `/after-sale/:orderNo` | 提交申请逻辑 | `POST /user/after-sale` | `UserAfterSaleService.apply` |
| 管理员售后裁决 | `/admin/after-sales` | `loadList`、`submitJudge` | `PUT /admin/after-sale/{id}/handle` | `AdminAfterSaleService.handle` |
...
6. 关键按钮通过“提交中”状态控制避免重复提交，后端对支付、退款、售后等关键状态流转再次执行状态校验，构成前后端双重保护。
